...
def write_document():
    if end_list_v2ray == [] or end_list_clash == []:
        ...
        for index in range(len(lines)):
            ...
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription2`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription3`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription4`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription5`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription6`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription7`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
            if lines[index] == '`https://raw.githubusercontent.com/w1770946466/Auto_proxy/main/Long_term_subscription8`': # 目标行内容
                lines.pop(index+1)
                lines.insert(index+1, f'合并节点总数: `{step-1}`\n')
                print("第八个写入完成✅")
